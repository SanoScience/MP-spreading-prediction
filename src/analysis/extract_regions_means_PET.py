...
class pet_loader(Thread):
    ...
    def extract_avg_region_means(self, pet_data):
        ...
        for label in atlas_labels:
            try:
                ...
                avg = pet_data[np.where(atlas_data == label)].mean()
    ...
    def run(self):  
        ...
        means = self.extract_avg_region_means(pet_data)
        ...
        if 'sub-CN' in self.pet:      
            ...
            if 'ses-baseline' in self.pet:
                cn_baseline_region.append(concentrations[self.pet])
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(pets))):
        pet_loader(pets[i]).start()

        # The currect thread counts in the active_count()
        while active_count() > num_cores+1:
            pass
    
    # NOTE: after starting the threading, the number of active threads will never go below 2 (Main and Monitor threads)
